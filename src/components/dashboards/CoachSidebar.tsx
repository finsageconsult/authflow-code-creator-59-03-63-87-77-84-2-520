import { NavLink, useLocation } from "react-router-dom"
import {
  Sidebar,
  SidebarContent,
  SidebarGroup,
  SidebarGroupContent,
  SidebarGroupLabel,
  SidebarMenu,
  SidebarMenuButton,
  SidebarMenuItem,
  SidebarHeader,
  useSidebar,
} from "@/components/ui/sidebar"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { X } from 'lucide-react'
import { 
  LayoutDashboard,
  Users,
  Calendar,
  MessageSquare,
  ClipboardList,
  Settings,
  TrendingUp,
  FileText,
  Coins,
  HelpCircle
} from 'lucide-react'

const menuItems = [
  { 
    title: "Overview", 
    url: "/coach-dashboard", 
    icon: LayoutDashboard,
    param: "overview"
  },
  { 
    title: "Sessions", 
    url: "/coach-dashboard", 
    icon: Calendar,
    param: "sessions"
  },
  { 
    title: "Chat", 
    url: "/coach-dashboard", 
    icon: MessageSquare,
    param: "chat"
  },
  { 
    title: "Clients", 
    url: "/coach-dashboard", 
    icon: Users,
    param: "clients"
  },
  { 
    title: "Analytics", 
    url: "/coach-dashboard", 
    icon: TrendingUp,
    param: "analytics"
  },
  { 
    title: "Assignments", 
    url: "/coach-dashboard", 
    icon: ClipboardList,
    param: "assignments"
  },
  { 
    title: "Content", 
    url: "/coach-dashboard", 
    icon: FileText,
    param: "content"
  },
  { 
    title: "Availability", 
    url: "/coach-dashboard", 
    icon: Settings,
    param: "availability"
  },
  { 
    title: "Payouts", 
    url: "/coach-dashboard", 
    icon: Coins,
    param: "payouts"
  },
  { 
    title: "Support", 
    url: "/coach-dashboard", 
    icon: HelpCircle,
    param: "support"
  }
]

export function CoachSidebar() {
  const { state, isMobile, setOpenMobile, openMobile } = useSidebar()
  const location = useLocation()
  const currentParam = new URLSearchParams(location.search).get('tab') || 'overview'

  const isActive = (param: string) => currentParam === param
  const isCollapsed = state === "collapsed"

  const handleItemClick = () => {
    if (isMobile && openMobile) {
      setOpenMobile(false)
    }
  }

  return (
    <Sidebar
      className="border-r bg-background/95 backdrop-blur-sm md:static md:w-48 lg:w-56 flex flex-col h-full"
      collapsible="icon"
      variant="sidebar"
      side="left"
    >
      {/* Mobile/Tablet header with close button */}
      <SidebarHeader className="flex flex-row items-center justify-between p-3 sm:p-4 border-b bg-background/95 lg:hidden">
        <div className="flex items-center gap-2 min-w-0">
          <div>
            <span className="font-semibold text-sm truncate">Coach Dashboard</span>
            <Badge variant="secondary" className="bg-orange-100 text-orange-800 text-xs mt-1 block sm:hidden">
              Coach  
            </Badge>
          </div>
        </div>
        <Button 
          variant="ghost" 
          size="sm" 
          onClick={() => {
            setOpenMobile(false);
          }} 
          className="h-8 w-8 p-0 hover:bg-muted shrink-0"
        >
          <X className="h-4 w-4" />
        </Button>
      </SidebarHeader>

      {/* Desktop header - hidden on mobile */}
      <SidebarHeader className="hidden lg:block p-2 border-b bg-background/95">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-base">Coach Dashboard</span>
        </div>
      </SidebarHeader>
      
      <SidebarContent className="flex-1 overflow-auto">
        <SidebarGroup>
          <SidebarGroupLabel className={`px-2 py-1 text-sm font-medium ${isCollapsed ? "sr-only" : ""}`}>
            Coach Dashboard
          </SidebarGroupLabel>
          
          <SidebarGroupContent className="px-1">
            <SidebarMenu className="space-y-1">
              {menuItems.map((item) => {
                const active = isActive(item.param)
                return (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton 
                      asChild
                      isActive={active}
                      className="w-full justify-start"
                    >
                      <NavLink
                        to={`${item.url}?tab=${item.param}`}
                        onClick={handleItemClick}
                        className={`flex items-center gap-2 px-2 py-2 rounded-lg transition-colors text-sm lg:text-base min-h-[40px] ${
                          active ? "bg-accent text-accent-foreground font-medium" : "hover:bg-accent/50"
                        }`}
                      >
                        <item.icon className="h-5 w-5 flex-shrink-0" />
                        <span className={`truncate ${isCollapsed ? "hidden" : ""}`}>{item.title}</span>
                      </NavLink>
                    </SidebarMenuButton>
                  </SidebarMenuItem>
                )
              })}
            </SidebarMenu>
          </SidebarGroupContent>
        </SidebarGroup>
      </SidebarContent>
    </Sidebar>
  )
}