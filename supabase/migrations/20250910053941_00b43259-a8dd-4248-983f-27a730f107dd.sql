...
WITH CHECK (
  ...
  -- Allow coaches to create chats 
  auth.uid() IN (
    SELECT coach_id FROM coach_profiles WHERE user_id = auth.uid()
  ) OR
  -- Allow individual users to create chats with their enrolled coaches
  auth.uid() IN (
    SELECT user_id FROM enrollments WHERE status = 'confirmed'
  )
);
...
USING (
  ...
  id IN (
    ...
    WHERE e.user_id = auth.uid() AND e.status = 'confirmed'
  )
);
