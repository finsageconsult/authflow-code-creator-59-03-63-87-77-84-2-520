...
CREATE TABLE public.payouts (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  payout_number TEXT NOT NULL UNIQUE DEFAULT ('PAYOUT' || to_char(NOW(), 'YYYYMM') || lpad((SELECT COUNT(*) + 1 FROM payouts WHERE created_at >= date_trunc('month', NOW()))::TEXT, 4, '0')),
  coach_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
...
CREATE POLICY "Coaches can view their own payout line items"
ON public.payout_line_items
FOR SELECT
USING (payout_id IN (
...
BEGIN
  ...
  purchase_data AS (
    ...
    WHERE pur.status = 'completed'
      ...
      AND ip.category = '1-1-sessions' -- Only 1-1 sessions are coach-specific
  )
  SELECT 
    COUNT(*)::INTEGER,
